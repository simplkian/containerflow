...
export function getApiUrl(): string {
  ...
  if (explicitUrl) {
    try {
      ...
      if (DEBUG_API) {
        console.log(`[API CONFIG] Base URL (EXPO_PUBLIC_API_URL): ${parsed.toString()}`);
      }
      return parsed.toString().replace(/\/$/, "");
    } catch (e) {
      console.warn(
        `[API CONFIG] EXPO_PUBLIC_API_URL is set but invalid (${explicitUrl}). Falling back to EXPO_PUBLIC_DOMAIN.`,
      );
    }
  }
  ...
  if (!host) {
    console.warn(
      ...
      "Set EXPO_PUBLIC_API_URL or EXPO_PUBLIC_DOMAIN to your API domain (e.g., 'https://your-domain.replit.dev:5000')",
    );
