...
/**
 * Box Placements Table
 * History of box placements on stands. removedAt NULL = active placement.
 */
export const boxPlacements = pgTable("box_placements", {
  id: varchar("id")
    .primaryKey()
    .default(sql`gen_random_uuid()`),
  ...
  uniqueActiveBox: uniqueIndex("uq_box_active")
    .on(table.boxId)
...
export const boxPlacementsRelations = relations(boxPlacements, ({ one }) => ({
  box: one(boxes, {
...
/**
 * Task Pickup Items
 * Defines the expected stands/boxes to be scanned for a task.
 */
export const taskPickupItems = pgTable("task_pickup_items", {
  id: varchar("id")
    .primaryKey()
    .default(sql`gen_random_uuid()`),
...
export const taskPickupItemsRelations = relations(taskPickupItems, ({ one, many }) => ({
  task: one(tasks, {
...
/**
 * Task Pickup Scans
 * Audit trail of each box scanned against a task pickup item.
 */
export const taskPickupScans = pgTable("task_pickup_scans", {
  id: varchar("id")
    .primaryKey()
    .default(sql`gen_random_uuid()`),
  taskId: varchar("task_id").notNull().references(() => tasks.id),
...
export const taskPickupScansRelations = relations(taskPickupScans, ({ one }) => ({
  task: one(tasks, {
...
export const insertBoxPlacementSchema = createInsertSchema(boxPlacements);
export const insertTaskPickupItemSchema = createInsertSchema(taskPickupItems);
export const insertTaskPickupScanSchema = createInsertSchema(taskPickupScans);
...
export type InsertBoxPlacement = z.infer<typeof insertBoxPlacementSchema>;
export type InsertTaskPickupItem = z.infer<typeof insertTaskPickupItemSchema>;
export type InsertTaskPickupScan = z.infer<typeof insertTaskPickupScanSchema>;
...
export type BoxPlacement = typeof boxPlacements.$inferSelect;
export type TaskPickupItem = typeof taskPickupItems.$inferSelect;
export type TaskPickupScan = typeof taskPickupScans.$inferSelect;
