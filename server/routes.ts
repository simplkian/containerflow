...
import { 
  materials, halls, stations, stands, boxes, boxPlacements, taskPickupItems, taskPickupScans, taskEvents, tasks, warehouseContainers, users, departments, taskSchedules,
  scanEvents, activityLogs,
  assertAutomotiveTransition, getAutomotiveTimestampFieldForStatus, isValidAutomotiveTransition, isValidTaskTransition, getTimestampFieldForStatus,
  type Material, type Hall, type Station, type Stand, type Box, type BoxPlacement, type TaskPickupItem, type TaskPickupScan, type TaskEvent, type TaskSchedule
} from "@shared/schema";
...
async function getActivePlacementForBox(boxId: string, tx: any = db): Promise<BoxPlacement | undefined> {
  const [placement] = await tx
    .select()
    .from(boxPlacements)
    .where(and(eq(boxPlacements.boxId, boxId), isNull(boxPlacements.removedAt)))
    .orderBy(desc(boxPlacements.placedAt))
    .limit(1);
  return placement || undefined;
}

async function getActivePlacementsForStand(standId: string, tx: any = db): Promise<BoxPlacement[]> {
  return tx
    .select()
    .from(boxPlacements)
    .where(and(eq(boxPlacements.standId, standId), isNull(boxPlacements.removedAt)))
    .orderBy(desc(boxPlacements.placedAt));
}

async function checkStandCapacity(stand: Stand, tx: any = db): Promise<{ ok: boolean; activeCount: number; maxSlots: number }> {
  const [{ total }] = await tx
    .select({ total: count() })
    .from(boxPlacements)
    .where(and(eq(boxPlacements.standId, stand.id), isNull(boxPlacements.removedAt)));
...
> {
  ...
  if (existingPlacement) {
    ...
    return {
      error: {
        status: 409,
        message: existingPlacement.standId === stand.id
          ? "Box ist bereits an diesem Stellplatz platziert."
          : "Diese Box ist bereits woanders platziert.",
  ...
  if (!ok) {
    return {
      error: {
        status: 409,
        message: "Dieser Stellplatz ist voll. Bitte erst Box entfernen oder max_slots erhöhen.",
...
async function removeActivePlacement({
  ...
  | { placement: BoxPlacement; updatedBox: Box }
  | { error: { status: number; message: string; context?: Record<string, unknown> } }
> {
  ...
  if (standId && placement.standId !== standId) {
    return {
      error: {
        status: 409,
        message: "Diese Box gehört aktuell nicht zu diesem Stellplatz/Ständer.",
        context: { currentStandId: placement.standId },
      },
    };
  }

  const [updatedPlacement] = await tx
    .update(boxPlacements)
    .set({
      removedAt: new Date(),
      removedByUserId: userId || null,
    })
    .where(and(eq(boxPlacements.id, placement.id), isNull(boxPlacements.removedAt)))
    .returning();
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Multi-pickup scan: validate placement, enforce stand expectations, update progress
  app.post("/api/tasks/:taskId/scan-box", requireAuth, async (req, res) => {
    try {
      ...
      const [pickupItem] = await db
        .select()
        .from(taskPickupItems)
        .where(and(eq(taskPickupItems.taskId, taskId), eq(taskPickupItems.standId, placement.standId)))
        .limit(1);
      ...
      const [placementStand] = await db.select().from(stands).where(eq(stands.id, placement.standId));
      if (pickupItem.materialId && placementStand?.materialId && pickupItem.materialId !== placementStand.materialId) {
        return res.status(422).json({ error: "Task erwartet anderes Material für diesen Stand" });
      }

      const [existingScan] = await db
        .select()
        .from(taskPickupScans)
        .where(and(eq(taskPickupScans.taskId, taskId), eq(taskPickupScans.boxId, box.id)))
        .limit(1);
      if (existingScan) {
        return res.status(409).json({ error: "Box wurde für diesen Task bereits gescannt" });
      }

      const [{ total: standScanCountRaw }] = await db
        .select({ total: count() })
        .from(taskPickupScans)
        .where(and(eq(taskPickupScans.taskId, taskId), eq(taskPickupScans.standId, placement.standId)));
      const standScannedCount = Number(standScanCountRaw || 0);

      if (standScannedCount >= pickupItem.requiredCount) {
        return res.status(409).json({ error: "Erforderliche Anzahl für diesen Stand ist bereits erreicht" });
      }
      ...
      const txResult: any = await db.transaction(async (tx) => {
        ...
        await tx
          .update(taskPickupItems)
          .set({
            scannedCount: standScannedCount + 1,
            updatedAt: new Date(),
          })
          .where(eq(taskPickupItems.id, pickupItem.id));

        if (taskId) {
          await tx.update(boxes).set({ currentTaskId: taskId, updatedAt: new Date() }).where(eq(boxes.id, box.id));
        }

        return {
          removal: {
            ...removal,
            updatedBox: { ...removal.updatedBox, currentTaskId: taskId },
          },
          scan,
        };
      });
      ...
      const pickupItems = await db.select().from(taskPickupItems).where(eq(taskPickupItems.taskId, taskId));
      ...
      if (allFulfilled && task.status !== "PICKED_UP") {
        const canTransition =
          ...
          isValidAutomotiveTransition(task.status, "PICKED_UP") ||
          isValidTaskTransition(task.status, "PICKED_UP");

        if (!canTransition) {
          return res
            .status(422)
            .json({ error: `Task-Status ${task.status} erlaubt kein PICKED_UP`, progress });
        }

        const previousStatus = task.status;
        const timestampField =
          getAutomotiveTimestampFieldForStatus("PICKED_UP") || getTimestampFieldForStatus("PICKED_UP");
      ...
      await db.insert(activityLogs).values({
        type: "TASK_PICKED_UP",
        action: "TASK_PICKUP_SCAN",
        message: `Box ${box.serial} von Stand ${placementStand?.identifier || placement.standId} für Task gescannt`,
        userId: authUser.id,
        taskId,
        containerId: box.id,
        metadata: {
  ...
  app.patch("/api/stands/:id", requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      if (materialId === undefined && isActive === undefined && stationId === undefined && maxSlots === undefined) {
      ...
      const updateData: any = { updatedAt: new Date() };
      let parsedMaxSlots: number | undefined;
      ...
      if (maxSlots !== undefined) {
        ...
        if (parsedMaxSlots !== existingStand.maxSlots) {
          ...
          changes.push(`Kapazität: ${existingStand.maxSlots} → ${parsedMaxSlots}`);
        }
      }
  ...
  app.post("/api/stands", requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      const resolvedMaxSlots = maxSlots !== undefined ? Number(maxSlots) : 1;
    ...
    try {
      ...
      const [stand] = await db.update(stands)
        .set({
          ...
          ...(isActive !== undefined && { isActive }),
          ...(maxSlots !== undefined && { maxSlots: parsedMaxSlots }),
          updatedAt: new Date(),
        })
        .where(eq(stands.id, req.params.id))
        .returning();
  ...
  // ----------------------------------------------------------------------------
  // STAND PLACEMENTS (GUARDED)
  // ----------------------------------------------------------------------------

  // POST /api/stands/:standId/place-box - Place a box on a stand with capacity + uniqueness guards
  app.post("/api/stands/:standId/place-box", requireAuth, async (req, res) => {
    try {
      ...
      if ("error" in placementResult) {
        return res
          .status(placementResult.error.status)
          .json({ error: placementResult.error.message, context: placementResult.error.context });
      }
  ...
  // POST /api/stands/:standId/remove-box - Remove a box from a stand (optional task pickup scan)
  app.post("/api/stands/:standId/remove-box", requireAuth, async (req, res) => {
    try {
      const { standId } = req.params;
      ...
      const txResult: any = await db.transaction(async (tx) => {
        let pickupScan: TaskPickupScan | null = null;
        ...
        if (taskId) {
          const [existingScan] = await tx
            ...
            .limit(1);
          if (existingScan) {
            return { error: { status: 409, message: "Box wurde für diesen Task bereits gescannt" } };
          }
        }
        ...
        const updatedRemoval =
          taskId && removal.updatedBox
            ? { ...removal, updatedBox: { ...removal.updatedBox, currentTaskId: taskId } }
            : removal;
  ...
  app.get("/api/boxes", async (req, res) => {
    try {
      ...
      if (standId) {
        conditions.push(eq(boxPlacements.standId, standId as string));
      }
      ...
      const query = db
        ...
        .leftJoin(
          boxPlacements,
          and(eq(boxPlacements.boxId, boxes.id), isNull(boxPlacements.removedAt)),
        )
        .leftJoin(stands, eq(boxPlacements.standId, stands.id));
  ...
  app.get("/api/admin/stands/:standId/boxes", requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      const result = await db
        ...
        .innerJoin(
          boxes,
          and(eq(boxPlacements.boxId, boxes.id), eq(boxes.isActive, true)),
        )
        .where(and(eq(boxPlacements.standId, standId), isNull(boxPlacements.removedAt)));
  ...
  app.post("/api/admin/stands/:standId/assign-box", requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      if (!capacity.ok) {
        return res.status(409).json({
          error: "Dieser Stellplatz ist voll. Bitte erst Box entfernen oder max_slots erhöhen.",
  ...
  app.post("/api/admin/stands/:standId/unassign-box", requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      const [stand] = await db.select().from(stands).where(eq(stands.id, standId));
      const beforeData = { standId: standId, status: box.status };
  ...
  app.get("/api/boxes/qr/:qrCode", async (req, res) => {
    try {
      ...
      const [stand] = placement
        ? await db.select().from(stands).where(eq(stands.id, placement.standId))
        : [null];
  ...
  app.post("/api/boxes", requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      if (standId) {
        return res.status(400).json({
          error: "Standplatz wird über Placements verwaltet. Bitte /api/stands/:id/place-box nutzen.",
        });
      }
  ...
  app.put("/api/automotive/tasks/:id/status", requireAuth, async (req, res) => {
    try {
      ...
      let boxRecord: Box | undefined;

      if (effectiveBoxId) {
        [boxRecord] = await db.select().from(boxes).where(eq(boxes.id, effectiveBoxId));
        if (!boxRecord) {
          return res.status(404).json({ error: "Box nicht gefunden" });
        }
      }
      ...
      let placementContextStandId: string | undefined;

      // Handle placement removal for PICKED_UP
      if (status === "PICKED_UP" && boxRecord) {
        const placement = await getActivePlacementForBox(boxRecord.id);
        if (!placement) {
          return res.status(400).json({ error: "Diese Box gehört aktuell nicht zu diesem Stellplatz/Ständer." });
        }
        if (task.standId && placement.standId !== task.standId) {
          const expected = await buildStandContextMeta(task.standId);
          return res.status(400).json({
            error: "Diese Box gehört aktuell nicht zu diesem Stellplatz/Ständer.",
      ...
      // Handle placement creation for DROPPED_OFF
      ...
      if (!task.claimedByUserId || isClaimExpired(task.claimedAt)) {
        ...
        await createAuditEvent({
          ...
          metaJson: {
            ...standMeta,
            boxId: task.boxId || effectiveBoxId || undefined,
            reason: "Auto-claimed before status transition",
          },
        });
      }
      ...
      // DISPOSED or CANCELLED: Release box linkage
      if ((status === "DISPOSED" || status === "CANCELLED") && boxRecord) {
        await db.update(boxes)
          ...
          .where(eq(boxes.id, boxRecord.id));
      }

      const eventMetaJson = {
        ...standMeta,
        boxId: effectiveBoxId || undefined,
        containerId: effectiveBoxId || undefined,
        targetWarehouseContainerId: updateData.targetWarehouseContainerId || undefined,
        autoClaimed,
        autoReleased,
      };
  ...
  // POST /api/automotive/tasks/:id/pickup-scan - Multi-box pickup flow
  app.post("/api/automotive/tasks/:id/pickup-scan", requireAuth, async (req, res) => {
    try {
      ...
      let stand;
      if (standId) {
        [stand] = await db.select().from(stands).where(eq(stands.id, standId));
      } else if (standQr) {
        [stand] = await db.select().from(stands).where(eq(stands.qrCode, standQr));
      }
      if (!stand) {
        return res.status(404).json({ error: "Stellplatz nicht gefunden" });
      }
      ...
      const [pickupItem] = await db
        .select()
        .from(taskPickupItems)
        .where(and(eq(taskPickupItems.taskId, task.id), eq(taskPickupItems.standId, stand.id)));

      if (!pickupItem) {
        const expected = await db
          .select({ identifier: stands.identifier })
          .from(taskPickupItems)
          .leftJoin(stands, eq(taskPickupItems.standId, stands.id))
          .where(eq(taskPickupItems.taskId, task.id));
        const expectedMsg =
          expected.length > 0
            ? expected.map((e) => e.identifier).filter(Boolean).join(", ")
            : "kein Stellplatz hinterlegt";
        return res.status(400).json({
      ...
      if (pickupItem.scannedCount >= pickupItem.requiredCount) {
        return res.status(409).json({
          error: `Dieser Stellplatz ist bereits vollständig gescannt (${pickupItem.scannedCount}/${pickupItem.requiredCount}).`,
        });
      }

      const duplicateScan = await db
        .select()
        .from(taskPickupScans)
        .where(and(eq(taskPickupScans.taskId, task.id), eq(taskPickupScans.boxId, box.id)))
        .limit(1);
      if (duplicateScan.length > 0) {
        return res.status(409).json({ error: "Diese Box wurde für diese Aufgabe bereits gescannt." });
      }
      ...
      const txResult = await db.transaction(async (tx) => {
        ...
        const remaining = await tx
          ...
          .where(
            and(
              eq(taskPickupItems.taskId, task.id),
              sql`${taskPickupItems.scannedCount} < ${taskPickupItems.requiredCount}`,
            ),
          );
      ...
      if (box.currentTaskId) {
        ...
        if (task && (task.status === "PICKED_UP" || task.status === "IN_TRANSIT")) {
          try {
            ...
            if (timestampField) {
              taskUpdate[timestampField] = new Date();
            }
  ...
  app.post("/api/scan/pickup-box", requireAuth, async (req, res) => {
    try {
      ...
      // If stand QR provided, ensure it matches placement
      if (standQr) {
        const [scannedStand] = await db.select().from(stands).where(eq(stands.qrCode, standQr));
        if (!scannedStand) {
          return res.status(404).json({ error: "Stellplatz nicht gefunden" });
        }
        if (scannedStand.id !== placement.standId) {
          return res.status(409).json({ error: "Diese Box gehört aktuell nicht zu diesem Stellplatz/Ständer." });
        }
      }
