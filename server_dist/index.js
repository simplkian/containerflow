...
__export(schema_exports, {
  ...
  boxPlacementsRelations: () => boxPlacementsRelations,
  ...
  insertBoxPlacementSchema: () => insertBoxPlacementSchema,
  ...
  insertTaskPickupItemSchema: () => insertTaskPickupItemSchema,
  insertTaskPickupScanSchema: () => insertTaskPickupScanSchema,
  ...
  taskPickupItems: () => taskPickupItems,
  taskPickupItemsRelations: () => taskPickupItemsRelations,
  taskPickupScans: () => taskPickupScans,
  taskPickupScansRelations: () => taskPickupScansRelations,
...
import { pgTable, text, varchar, integer, boolean, timestamp, real, jsonb, pgEnum, uniqueIndex, index } from "drizzle-orm/pg-core";
...
var boxPlacements = pgTable("box_placements", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  ...
  uniqueActiveBox: uniqueIndex("uq_box_active").on(table.boxId).where(sql`${table.removedAt} IS NULL`),
  boxIdx: index("idx_box").on(table.boxId),
  standIdx: index("idx_stand").on(table.standId)
}));
var boxPlacementsRelations = relations(boxPlacements, ({ one }) => ({
  box: one(boxes, {
    fields: [boxPlacements.boxId],
    references: [boxes.id]
  }),
  stand: one(stands, {
    fields: [boxPlacements.standId],
    references: [stands.id]
  }),
  placedBy: one(users, {
    fields: [boxPlacements.placedByUserId],
    references: [users.id]
  }),
  removedBy: one(users, {
    fields: [boxPlacements.removedByUserId],
    references: [users.id]
  })
}));
...
var tasksRelations = relations(tasks, ({ one, many }) => ({
  ...
  pickupItems: many(taskPickupItems),
  pickupScans: many(taskPickupScans),
  scanEvents: many(scanEvents),
  activityLogs: many(activityLogs),
  fillHistory: many(fillHistory),
  taskEvents: many(taskEvents)
}));
var taskPickupItems = pgTable("task_pickup_items", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  ...
  taskStandUnique: uniqueIndex("task_pickup_items_task_stand_unique").on(
    table.taskId,
    table.standId
  )
}));
var taskPickupItemsRelations = relations(taskPickupItems, ({ one, many }) => ({
  task: one(tasks, {
    fields: [taskPickupItems.taskId],
    references: [tasks.id]
  }),
  stand: one(stands, {
    fields: [taskPickupItems.standId],
    references: [stands.id]
  }),
  material: one(materials, {
    fields: [taskPickupItems.materialId],
    references: [materials.id]
  }),
  scans: many(taskPickupScans)
}));
var taskPickupScans = pgTable("task_pickup_scans", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  taskId: varchar("task_id").notNull().references(() => tasks.id),
  standId: varchar("stand_id").notNull().references(() => stands.id),
  boxId: varchar("box_id").notNull().references(() => boxes.id),
  ...
  uniqueBoxPerTask: uniqueIndex("uq_task_box").on(table.taskId, table.boxId)
}));
var taskPickupScansRelations = relations(taskPickupScans, ({ one }) => ({
  task: one(tasks, {
    fields: [taskPickupScans.taskId],
    references: [tasks.id]
  }),
  stand: one(stands, {
    fields: [taskPickupScans.standId],
    references: [stands.id]
  }),
  box: one(boxes, {
    fields: [taskPickupScans.boxId],
    references: [boxes.id]
  }),
  scannedBy: one(users, {
    fields: [taskPickupScans.scannedByUserId],
    references: [users.id]
  })
}));
...
var insertBoxPlacementSchema = createInsertSchema(boxPlacements);
var insertTaskPickupItemSchema = createInsertSchema(taskPickupItems);
var insertTaskPickupScanSchema = createInsertSchema(taskPickupScans);
...
async function getActivePlacementForBox(boxId, tx = db) {
  const [placement] = await tx.select().from(boxPlacements).where(and2(eq2(boxPlacements.boxId, boxId), isNull(boxPlacements.removedAt))).orderBy(desc2(boxPlacements.placedAt)).limit(1);
  return placement || void 0;
}
async function getActivePlacementsForStand(standId, tx = db) {
  return tx.select().from(boxPlacements).where(and2(eq2(boxPlacements.standId, standId), isNull(boxPlacements.removedAt))).orderBy(desc2(boxPlacements.placedAt));
}
async function checkStandCapacity(stand, tx = db) {
  const [{ total }] = await tx.select({ total: count() }).from(boxPlacements).where(and2(eq2(boxPlacements.standId, stand.id), isNull(boxPlacements.removedAt)));
  const activeCount = Number(total || 0);
  const maxSlots = Math.max(1, stand.maxSlots || 1);
  return { ok: activeCount < maxSlots, activeCount, maxSlots };
}
async function placeBoxWithGuards({
  box,
  stand,
  userId,
  positionIndex,
  allowSameStand = false,
  taskId,
  tx = db
}) {
  const existingPlacement = await getActivePlacementForBox(box.id, tx);
  if (existingPlacement) {
    if (allowSameStand && existingPlacement.standId === stand.id) {
      const [currentBox] = await tx.select().from(boxes).where(eq2(boxes.id, box.id)).limit(1);
      return {
        placement: existingPlacement,
        updatedBox: currentBox || box,
        alreadyPlaced: true,
        activeCount: (await checkStandCapacity(stand, tx)).activeCount,
        maxSlots: stand.maxSlots || 1
      };
    }
    return {
      error: {
        status: 409,
        message: existingPlacement.standId === stand.id ? "Box ist bereits an diesem Stellplatz platziert." : "Diese Box ist bereits woanders platziert.",
        context: { currentStandId: existingPlacement.standId }
      }
    };
  }
  const { ok, activeCount, maxSlots } = await checkStandCapacity(stand, tx);
  if (!ok) {
    return {
      error: {
        status: 409,
        message: "Dieser Stellplatz ist voll. Bitte erst Box entfernen oder max_slots erh\xF6hen.",
        context: { activeCount, maxSlots }
      }
    };
  }
  const [placement] = await tx.insert(boxPlacements).values({
    boxId: box.id,
    standId: stand.id,
  ...
  const [updatedBox] = await tx.update(boxes).set({
    standId: stand.id,
    status: "AT_STAND",
    lastSeenAt: /* @__PURE__ */ new Date(),
    updatedAt: /* @__PURE__ */ new Date(),
    ...taskId !== void 0 && { currentTaskId: taskId }
  }).where(eq2(boxes.id, box.id)).returning();
  return { placement, updatedBox: updatedBox || box, activeCount: activeCount + 1, maxSlots };
}
async function removeActivePlacement({
  ...
  const placement = await getActivePlacementForBox(box.id, tx);
  if (!placement) {
    return {
      error: {
        status: 409,
        message: "Diese Box hat aktuell keine aktive Platzierung."
      }
    };
  }
  if (standId && placement.standId !== standId) {
    return {
      error: {
        status: 409,
        message: "Diese Box geh\xF6rt aktuell nicht zu diesem Stellplatz/St\xE4nder.",
        context: { currentStandId: placement.standId }
      }
    };
  }
  const [updatedPlacement] = await tx.update(boxPlacements).set({
    removedAt: /* @__PURE__ */ new Date(),
    removedByUserId: userId || null
  }).where(and2(eq2(boxPlacements.id, placement.id), isNull(boxPlacements.removedAt))).returning();
  const [updatedBox] = await tx.update(boxes).set({
    standId: null,
    status: "IN_TRANSIT",
    lastSeenAt: /* @__PURE__ */ new Date(),
  ...
  return { placement: updatedPlacement, updatedBox: updatedBox || box };
}
...
async function registerRoutes(app2) {
  ...
  app2.post("/api/warehouse-containers/:id/empty", requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      await storage.createActivityLog({
        ...
        message: `Warehouse container ${container.location} geleert`,
        userId: authUser.id,
  ...
  app2.post("/api/tasks/:id/accept", async (req, res) => {
    try {
      ...
      if (!task.containerID) {
        return res.status(400).json({ error: "Task hat keinen Kundencontainer" });
      }
      const user = await storage.getUser(userId);
      if (!user) {
  ...
  app2.post("/api/tasks/:taskId/scan-box", requireAuth, async (req, res) => {
    try {
      const { taskId } = req.params;
      const { boxQr } = req.body;
      const authUser = req.authUser;
      if (!boxQr) {
        return res.status(400).json({ error: "boxQr ist erforderlich" });
      }
      let [task] = await db.select().from(tasks).where(eq2(tasks.id, taskId));
      if (!task) {
      ...
      const placement = await getActivePlacementForBox(box.id);
      if (!placement) {
        return res.status(409).json({ error: "Box ist aktuell nicht platziert" });
      }
      const [pickupItem] = await db.select().from(taskPickupItems).where(and2(eq2(taskPickupItems.taskId, taskId), eq2(taskPickupItems.standId, placement.standId))).limit(1);
      if (!pickupItem) {
        return res.status(422).json({ error: "Task erwartet einen anderen Stand" });
      }
      const [placementStand] = await db.select().from(stands).where(eq2(stands.id, placement.standId));
      if (pickupItem.materialId && placementStand?.materialId && pickupItem.materialId !== placementStand.materialId) {
        return res.status(422).json({ error: "Task erwartet anderes Material f\xFCr diesen Stand" });
      }
      const [existingScan] = await db.select().from(taskPickupScans).where(and2(eq2(taskPickupScans.taskId, taskId), eq2(taskPickupScans.boxId, box.id))).limit(1);
      if (existingScan) {
        return res.status(409).json({ error: "Box wurde f\xFCr diesen Task bereits gescannt" });
      }
      const [{ total: standScanCountRaw }] = await db.select({ total: count() }).from(taskPickupScans).where(and2(eq2(taskPickupScans.taskId, taskId), eq2(taskPickupScans.standId, placement.standId)));
      const standScannedCount = Number(standScanCountRaw || 0);
      if (standScannedCount >= pickupItem.requiredCount) {
        return res.status(409).json({ error: "Erforderliche Anzahl f\xFCr diesen Stand ist bereits erreicht" });
      }
      const txResult = await db.transaction(async (tx) => {
        const removal2 = await removeActivePlacement({
          box,
          standId: placement.standId,
          userId: authUser.id,
          tx
        });
        if ("error" in removal2) {
          return { error: removal2.error };
        }
        const [scan2] = await tx.insert(taskPickupScans).values({
          taskId,
          standId: placement.standId,
          boxId: box.id,
          scannedByUserId: authUser.id
        }).returning();
        await tx.update(taskPickupItems).set({
          scannedCount: standScannedCount + 1,
          updatedAt: /* @__PURE__ */ new Date()
        }).where(eq2(taskPickupItems.id, pickupItem.id));
        if (taskId) {
          await tx.update(boxes).set({ currentTaskId: taskId, updatedAt: /* @__PURE__ */ new Date() }).where(eq2(boxes.id, box.id));
        }
        return {
          removal: {
            ...removal2,
            updatedBox: { ...removal2.updatedBox, currentTaskId: taskId }
          },
          scan: scan2
        };
      });
      const { removal, scan } = txResult;
      const pickupItems = await db.select().from(taskPickupItems).where(eq2(taskPickupItems.taskId, taskId));
      const progress = pickupItems.map((item) => ({
        standId: item.standId,
        required: item.requiredCount,
        scanned: item.scannedCount
      }));
      const allFulfilled = progress.every((p) => (p.scanned || 0) >= p.required);
      const standMeta = await buildStandContextMeta(placement.standId);
      await createAuditEvent({
        taskId,
        actorUserId: authUser.id,
        action: "TASK_PICKUP_SCAN",
        entityType: "box",
        entityId: box.id,
        beforeData: { placementId: placement.id },
        afterData: { placementId: null, standId: placement.standId },
        metaJson: { ...standMeta, boxId: box.id, scanId: scan.id, source: "TASK_SCAN_BOX" }
      });
      if (allFulfilled && task.status !== "PICKED_UP") {
        const canTransition = task.status === "PICKED_UP" || isValidAutomotiveTransition(task.status, "PICKED_UP") || isValidTaskTransition(task.status, "PICKED_UP");
        if (!canTransition) {
          return res.status(422).json({ error: `Task-Status ${task.status} erlaubt kein PICKED_UP`, progress });
        }
        const previousStatus = task.status;
        const timestampField = getAutomotiveTimestampFieldForStatus("PICKED_UP") || getTimestampFieldForStatus("PICKED_UP");
        const updateData = {
          status: "PICKED_UP",
          updatedAt: /* @__PURE__ */ new Date(),
        ...
        const [updatedTask] = await db.update(tasks).set(updateData).where(eq2(tasks.id, taskId)).returning();
        task = updatedTask;
        await createAuditEvent({
          taskId,
          actorUserId: authUser.id,
          ...
          afterData: { status: "PICKED_UP" },
          metaJson: { ...standMeta, boxId: box.id, source: "TASK_SCAN_BOX" }
        });
      }
      await db.insert(activityLogs).values({
        type: "TASK_PICKED_UP",
        action: "TASK_PICKUP_SCAN",
        message: `Box ${box.serial} von Stand ${placementStand?.identifier || placement.standId} f\xFCr Task gescannt`,
        userId: authUser.id,
        taskId,
        containerId: box.id,
        metadata: {
      ...
      res.json({
        scan,
        removedPlacement: removal.placement,
        box: removal.updatedBox,
        taskStatus: task.status,
        progress,
        allFulfilled
      });
      ...
      console.error("[TaskPickup] Failed to scan box:", error);
  ...
  app2.patch("/api/stations/:id", requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      await db.insert(activityLogs).values({
        ...
        message: `Station ${existingStation.code} wurde verschoben`,
        userId: authUser.id,
        containerId: null,
        taskId: null,
        scanEventId: null,
  ...
  app2.patch("/api/stands/:id", requireAuth, requireAdmin, async (req, res) => {
    try {
      const { materialId, isActive, stationId, maxSlots } = req.body;
      const authUser = req.authUser;
      if (materialId === void 0 && isActive === void 0 && stationId === void 0 && maxSlots === void 0) {
      ...
      if (maxSlots !== void 0) {
        parsedMaxSlots = Number(maxSlots);
        if (!Number.isFinite(parsedMaxSlots) || parsedMaxSlots < 1) {
          return res.status(400).json({ error: "maxSlots muss >= 1 sein" });
        }
        const activePlacementCount = (await getActivePlacementsForStand(existingStand.id)).length;
        if (parsedMaxSlots < activePlacementCount) {
          return res.status(409).json({
            error: "maxSlots kann nicht kleiner als aktuelle Belegung sein",
            activeCount: activePlacementCount,
            maxSlots: parsedMaxSlots
          });
        }
        if (parsedMaxSlots !== existingStand.maxSlots) {
          updateData.maxSlots = parsedMaxSlots;
          changes.push(`Kapazit\xE4t: ${existingStand.maxSlots} \u2192 ${parsedMaxSlots}`);
        }
      }
      ...
      await db.insert(activityLogs).values({
        type: "SYSTEM_EVENT",
        action: "STAND_EDITED",
        message: `Stellplatz ${existingStand.identifier} aktualisiert`,
        userId: authUser.id,
        taskId: null,
        containerId: null,
        scanEventId: null,
  ...
  app2.post("/api/stands", requireAuth, requireAdmin, async (req, res) => {
    try {
      const { stationId, identifier, materialId, qrCode, sequence, positionMeta, dailyFull, maxSlots } = req.body;
      if (!stationId || !identifier || !qrCode) {
        return res.status(400).json({ error: "stationId, identifier, and qrCode are required" });
      }
      const resolvedMaxSlots = maxSlots !== void 0 ? Number(maxSlots) : 1;
      if (!Number.isFinite(resolvedMaxSlots) || resolvedMaxSlots < 1) {
        return res.status(400).json({ error: "maxSlots muss >= 1 sein" });
      }
  ...
  app2.put("/api/stands/:id", requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      const { identifier, materialId, qrCode, sequence, positionMeta, dailyFull, isActive, maxSlots } = req.body;
      let parsedMaxSlots;
      if (maxSlots !== void 0) {
        parsedMaxSlots = Number(maxSlots);
        if (!Number.isFinite(parsedMaxSlots) || parsedMaxSlots < 1) {
        ...
        const activePlacementCount = (await getActivePlacementsForStand(existing.id)).length;
        if (parsedMaxSlots < activePlacementCount) {
          return res.status(409).json({
            error: "maxSlots kann nicht kleiner als aktuelle Belegung sein",
            activeCount: activePlacementCount,
      ...
      const [stand] = await db.update(stands).set({
        ...
        ...isActive !== void 0 && { isActive },
        ...maxSlots !== void 0 && { maxSlots: parsedMaxSlots },
        updatedAt: /* @__PURE__ */ new Date()
  ...
  app2.patch("/api/automotive/stands/:id", requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      const { dailyFull, dailyTaskTimeLocal, identifier, materialId, qrCode, sequence, positionMeta, isActive, maxSlots } = req.body;
      let parsedMaxSlots;
      if (maxSlots !== void 0) {
        parsedMaxSlots = Number(maxSlots);
        if (!Number.isFinite(parsedMaxSlots) || parsedMaxSlots < 1) {
  ...
  app2.post("/api/stands/:standId/place-box", requireAuth, async (req, res) => {
    try {
      const { standId } = req.params;
      const { boxQr, boxId, positionIndex } = req.body;
      const authUser = req.authUser;
      if (!boxQr && !boxId) {
        return res.status(400).json({ error: "boxQr oder boxId ist erforderlich" });
      }
      const [stand] = await db.select().from(stands).where(eq2(stands.id, standId));
      if (!stand) {
        return res.status(404).json({ error: "Stellplatz nicht gefunden" });
      ...
      if (!stand.isActive) {
        return res.status(409).json({ error: "Stellplatz ist inaktiv" });
      }
      let box;
      if (boxId) {
      ...
      const existingPlacement = await getActivePlacementForBox(box.id);
      if (existingPlacement) {
        return res.status(409).json({
          error: "Box ist bereits platziert",
          standId: existingPlacement.standId
        });
      }
      const { ok, activeCount, maxSlots } = await checkStandCapacity(stand);
      if (!ok) {
        return res.status(409).json({
          error: "Stellplatz ist voll",
          activeCount,
          maxSlots
        });
      }
      const placementResult = await placeBoxWithGuards({
        box,
        stand,
        userId: authUser.id,
        positionIndex
      ...
      if ("error" in placementResult) {
        return res.status(placementResult.error.status).json({ error: placementResult.error.message, context: placementResult.error.context });
      }
      const { placement, updatedBox, activeCount: newActiveCount, maxSlots: maxSlotsAfter } = placementResult;
      const standMeta = await buildStandContextMeta(stand.id);
      await createAuditEvent({
        ...
        entityType: "box",
        entityId: box.id,
        beforeData: { standId: box.standId, status: box.status },
        afterData: { standId: stand.id, status: "AT_STAND" },
        metaJson: { ...standMeta, boxId: box.id, placementId: placement.id, source: "PLACE_BOX" }
      });
      await db.insert(activityLogs).values({
        type: "CONTAINER_STATUS_CHANGED",
        action: "BOX_PLACED",
      ...
      res.status(201).json({
        ...
        box: updatedBox,
        stand,
        capacity: { activeCount: newActiveCount, maxSlots: maxSlotsAfter }
      });
    } catch (error) {
      console.error("[Placements] Failed to place box:", error);
      res.status(500).json({ error: "Platzierung fehlgeschlagen" });
    }
  });
  app2.post("/api/stands/:standId/remove-box", requireAuth, async (req, res) => {
    try {
      const { standId } = req.params;
      const { boxQr, boxId, taskId } = req.body;
      const authUser = req.authUser;
      if (!boxQr && !boxId) {
        return res.status(400).json({ error: "boxQr oder boxId ist erforderlich" });
      }
      ...
      const placement = await getActivePlacementForBox(box.id);
      if (!placement || placement.standId !== standId) {
        return res.status(409).json({ error: "Box ist aktuell nicht auf diesem Stellplatz platziert" });
      }
      let task = null;
      if (taskId) {
        [task] = await db.select().from(tasks).where(eq2(tasks.id, taskId));
        if (!task) {
          return res.status(404).json({ error: "Task nicht gefunden" });
        }
      }
      const txResult = await db.transaction(async (tx) => {
        let pickupScan = null;
        if (taskId) {
          const [existingScan] = await tx.select().from(taskPickupScans).where(and2(eq2(taskPickupScans.taskId, taskId), eq2(taskPickupScans.boxId, box.id))).limit(1);
          if (existingScan) {
            return { error: { status: 409, message: "Box wurde f\xFCr diesen Task bereits gescannt" } };
          }
        }
        const removal = await removeActivePlacement({
          box,
          standId,
          userId: authUser.id,
          tx
        ...
        if (taskId) {
          const [scan] = await tx.insert(taskPickupScans).values({
            taskId,
            standId,
            boxId: box.id,
            scannedByUserId: authUser.id
        ...
        const updatedRemoval = taskId && removal.updatedBox ? { ...removal, updatedBox: { ...removal.updatedBox, currentTaskId: taskId } } : removal;
        if (taskId) {
          await tx.update(boxes).set({ currentTaskId: taskId, updatedAt: /* @__PURE__ */ new Date() }).where(eq2(boxes.id, box.id));
        }
        return { removal: updatedRemoval, pickupScan };
      });
      if (txResult && txResult.error) {
        return res.status(txResult.error.status || 400).json({ error: txResult.error.message });
      }
      const { placement: removedPlacement, updatedBox } = txResult.removal;
      const standMeta = await buildStandContextMeta(stand.id);
      await createAuditEvent({
        taskId: taskId || box.currentTaskId || box.id,
        actorUserId: authUser.id,
        action: "BOX_REMOVED",
        entityType: "box",
        entityId: box.id,
        beforeData: { standId, placementId: placement.id },
        afterData: { standId: null, placementId: removedPlacement.id },
        metaJson: {
          ...standMeta,
          boxId: box.id,
          placementId: placement.id,
          removedPlacementId: removedPlacement.id,
          source: "REMOVE_BOX"
        }
      });
      await db.insert(activityLogs).values({
        type: "CONTAINER_STATUS_CHANGED",
        action: "BOX_REMOVED",
        message: `Box ${box.serial} wurde von Stellplatz ${stand.identifier} abgeholt`,
        userId: authUser.id,
        containerId: box.id,
        taskId: taskId || box.currentTaskId || null,
        metadata: {
          standId: stand.id,
          placementId: removedPlacement.id,
          taskId: taskId || null
        }
      });
      res.json({
        ...
        box: updatedBox,
        stand,
        taskScan: txResult.pickupScan || null
      });
    } catch (error) {
      if (error?.code === "23505") {
        return res.status(409).json({ error: "Box wurde f\xFCr diesen Task bereits gescannt" });
      }
      console.error("[Placements] Failed to remove box:", error);
  ...
  app2.get("/api/boxes", async (req, res) => {
    try {
      ...
      if (standId) {
        conditions.push(eq2(boxPlacements.standId, standId));
      }
      const query = db.select({
        box: boxes,
        activePlacement: boxPlacements,
        stand: stands
      }).from(boxes).leftJoin(
        boxPlacements,
        and2(eq2(boxPlacements.boxId, boxes.id), isNull(boxPlacements.removedAt))
      ).leftJoin(stands, eq2(boxPlacements.standId, stands.id));
      const result = conditions.length > 0 ? await query.where(and2(...conditions)) : await query;
      res.json(
        result.map((row) => ({
          ...row.box,
          standId: row.stand?.id || null,
          activePlacement: row.activePlacement || null,
  ...
  app2.get("/api/admin/stands/:standId/boxes", requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      }).from(boxPlacements).innerJoin(
        boxes,
        and2(eq2(boxPlacements.boxId, boxes.id), eq2(boxes.isActive, true))
      ).where(and2(eq2(boxPlacements.standId, standId), isNull(boxPlacements.removedAt)));
      res.json(
        result.map((row) => ({
          ...row.box,
          activePlacement: row.placement,
          standId
        }))
      );
    } catch (error) {
  ...
  app2.post("/api/admin/stands/:standId/assign-box", requireAuth, requireAdmin, async (req, res) => {
    try {
      const { standId } = req.params;
      const { boxId, boxSerial, boxQr, positionIndex } = req.body;
      const authUser = req.authUser;
      ...
      const existingPlacement = await getActivePlacementForBox(box.id);
      if (existingPlacement) {
        if (existingPlacement.standId === stand.id) {
          return res.json({
            message: "Box ist bereits an diesem Stellplatz",
            box,
            alreadyAssigned: true,
            activePlacement: existingPlacement
          });
        }
        return res.status(409).json({
          error: "Diese Box ist bereits woanders platziert.",
          currentStandId: existingPlacement.standId
        });
      }
      const capacity = await checkStandCapacity(stand);
      if (!capacity.ok) {
        return res.status(409).json({
          error: "Dieser Stellplatz ist voll. Bitte erst Box entfernen oder max_slots erh\xF6hen.",
          activeCount: capacity.activeCount,
          maxSlots: capacity.maxSlots
        });
      }
      const placed = await placeBoxWithGuards({
        box,
        stand,
        userId: authUser.id,
        positionIndex
      ...
      if ("error" in placed) {
        return res.status(placed.error.status).json({ error: placed.error.message });
      }
      await db.insert(activityLogs).values({
        type: "CONTAINER_STATUS_CHANGED",
        action: "BOX_PLACED",
        message: `Box ${box.serial} am Stellplatz ${stand.identifier} platziert`,
        userId: authUser.id,
        containerId: box.id,
        taskId: null,
        scanEventId: null,
  ...
  app2.post("/api/admin/stands/:standId/unassign-box", requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      if ("error" in removal) {
        return res.status(removal.error.status).json({ error: removal.error.message });
      }
      const [stand] = await db.select().from(stands).where(eq2(stands.id, standId));
      const beforeData = { standId, status: box.status };
      await db.insert(activityLogs).values({
        type: "CONTAINER_STATUS_CHANGED",
        action: "BOX_UNPLACED",
        message: `Box ${box.serial} vom Stellplatz entfernt`,
        userId: authUser.id,
        containerId: boxId,
        taskId: null,
        scanEventId: null,
  ...
  app2.get("/api/boxes/qr/:qrCode", async (req, res) => {
    try {
      ...
      const placement = await getActivePlacementForBox(box.id);
      const [stand] = placement ? await db.select().from(stands).where(eq2(stands.id, placement.standId)) : [null];
      res.json({ ...box, activePlacement: placement || null, stand: stand || null, standId: stand?.id || null });
    } catch (error) {
  ...
  app2.post("/api/boxes", requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      if (standId) {
        return res.status(400).json({
          error: "Standplatz wird \xFCber Placements verwaltet. Bitte /api/stands/:id/place-box nutzen."
        });
      }
  ...
  app2.post("/api/boxes/:id/position", requireAuth, async (req, res) => {
    try {
      const { standId, positionIndex } = req.body;
      const authUser = req.authUser;
      if (!standId) {
      ...
      if ("error" in placementResult) {
        const status = placementResult.error.status || 400;
        return res.status(status).json({ error: placementResult.error.message });
      }
      const updatedBox = placementResult.updatedBox;
      const standMeta = await buildStandContextMeta(standId);
      const eventMetaJson = {
        ...standMeta,
        boxId: box.id,
      ...
      await createAuditEvent({
        ...
        actorUserId: authUser.id,
        action: placementResult.alreadyPlaced ? "BOX_POSITIONED" : "PLACEMENT_CHANGED",
        entityType: "box",
        entityId: box.id,
        beforeData: { standId: box.standId, status: box.status },
        afterData: { standId, status: "AT_STAND" },
  ...
  app2.put("/api/automotive/tasks/:id/status", requireAuth, async (req, res) => {
    try {
      ...
      let boxRecord;
      if (effectiveBoxId) {
        [boxRecord] = await db.select().from(boxes).where(eq2(boxes.id, effectiveBoxId));
        if (!boxRecord) {
          return res.status(404).json({ error: "Box nicht gefunden" });
        }
      }
      let standMeta = task.standId ? await buildStandContextMeta(task.standId) : {};
      let placementContextStandId;
      if (status === "PICKED_UP" && boxRecord) {
        const placement = await getActivePlacementForBox(boxRecord.id);
        if (!placement) {
          return res.status(400).json({ error: "Diese Box geh\xF6rt aktuell nicht zu diesem Stellplatz/St\xE4nder." });
        }
        if (task.standId && placement.standId !== task.standId) {
          const expected = await buildStandContextMeta(task.standId);
          return res.status(400).json({
            error: "Diese Box geh\xF6rt aktuell nicht zu diesem Stellplatz/St\xE4nder.",
            expectedStandId: task.standId,
            currentStandId: placement.standId,
            expected
          });
        }
        const removal = await removeActivePlacement({
        ...
        boxRecord = removal.updatedBox;
        placementContextStandId = placement.standId;
        standMeta = await buildStandContextMeta(placement.standId);
      }
      if (status === "DROPPED_OFF" && boxRecord && scannedStandId) {
        const [scannedStand] = await db.select().from(stands).where(eq2(stands.id, scannedStandId));
        if (!scannedStand) {
          return res.status(404).json({ error: "Gescannter Stand nicht gefunden" });
        }
        const activePlacement = await getActivePlacementForBox(boxRecord.id);
        if (activePlacement) {
          await removeActivePlacement({
            box: boxRecord,
            standId: activePlacement.standId,
            userId: authUser.id,
            taskId: task.id
          });
        }
        ...
        if ("error" in placementResult) {
          return res.status(placementResult.error.status).json({ error: placementResult.error.message });
        }
        boxRecord = placementResult.updatedBox;
        placementContextStandId = scannedStand.id;
        standMeta = await buildStandContextMeta(scannedStand.id);
      }
      if (!task.claimedByUserId || isClaimExpired(task.claimedAt)) {
        autoClaimed = true;
        await createAuditEvent({
          ...
          metaJson: {
            ...standMeta,
            boxId: task.boxId || effectiveBoxId || void 0,
            reason: "Auto-claimed before status transition"
          }
        });
      }
      ...
      if (status === "PICKED_UP" && boxRecord && placementContextStandId) {
        await createAuditEvent({
          taskId: updatedTask.id,
          actorUserId: authUser.id,
          action: "BOX_REMOVED",
          entityType: "box",
          entityId: boxRecord.id,
          beforeData: { standId: placementContextStandId, status: "AT_STAND" },
          afterData: { standId: null, status: "IN_TRANSIT" },
          metaJson: {
            ...standMeta,
            boxId: boxRecord.id,
      ...
      if (status === "DROPPED_OFF" && boxRecord && placementContextStandId) {
        const dropStandMeta = await buildStandContextMeta(placementContextStandId);
        await createAuditEvent({
          taskId: updatedTask.id,
          actorUserId: authUser.id,
          action: "BOX_PLACED",
          ...
          afterData: { standId: placementContextStandId, status: "AT_STAND" },
          metaJson: {
            ...dropStandMeta,
            boxId: boxRecord.id,
            source: "DROPOFF_SCAN"
      ...
      if ((status === "DISPOSED" || status === "CANCELLED") && boxRecord) {
        await db.update(boxes).set({
          currentTaskId: null,
          status: status === "DISPOSED" ? "AT_WAREHOUSE" : boxRecord.status,
          updatedAt: now
        }).where(eq2(boxes.id, boxRecord.id));
      }
      const eventMetaJson = {
        ...standMeta,
        boxId: effectiveBoxId || void 0,
        containerId: effectiveBoxId || void 0,
        targetWarehouseContainerId: updateData.targetWarehouseContainerId || void 0,
        autoClaimed,
        autoReleased
      };
  ...
  app2.post("/api/automotive/tasks/:id/pickup-scan", requireAuth, async (req, res) => {
    try {
      const { standId, standQr, boxId, boxQr } = req.body;
      const authUser = req.authUser;
      if (!standId && !standQr) {
        return res.status(400).json({ error: "Stand-Scan erforderlich" });
      }
      if (!boxId && !boxQr) {
        return res.status(400).json({ error: "Box-Scan erforderlich" });
      }
      ...
      if (!["OPEN", "PICKED_UP"].includes(task.status)) {
        return res.status(409).json({ error: `Pickup-Scan nicht erlaubt im Status ${task.status}` });
      }
      let stand;
      if (standId) {
        [stand] = await db.select().from(stands).where(eq2(stands.id, standId));
      } else if (standQr) {
        [stand] = await db.select().from(stands).where(eq2(stands.qrCode, standQr));
      }
      if (!stand) {
        return res.status(404).json({ error: "Stellplatz nicht gefunden" });
      }
      ...
      const placement = await getActivePlacementForBox(box.id);
      if (!placement || placement.standId !== stand.id) {
        return res.status(400).json({ error: "Diese Box geh\xF6rt aktuell nicht zu diesem Stellplatz/St\xE4nder." });
      }
      const [pickupItem] = await db.select().from(taskPickupItems).where(and2(eq2(taskPickupItems.taskId, task.id), eq2(taskPickupItems.standId, stand.id)));
      if (!pickupItem) {
        const expected = await db.select({ identifier: stands.identifier }).from(taskPickupItems).leftJoin(stands, eq2(taskPickupItems.standId, stands.id)).where(eq2(taskPickupItems.taskId, task.id));
        const expectedMsg = expected.length > 0 ? expected.map((e) => e.identifier).filter(Boolean).join(", ") : "kein Stellplatz hinterlegt";
        return res.status(400).json({
          error: `Dieser Task erwartet Boxen von: ${expectedMsg}.`
        });
      }
      if (pickupItem.scannedCount >= pickupItem.requiredCount) {
        return res.status(409).json({
          error: `Dieser Stellplatz ist bereits vollst\xE4ndig gescannt (${pickupItem.scannedCount}/${pickupItem.requiredCount}).`
        });
      }
      const duplicateScan = await db.select().from(taskPickupScans).where(and2(eq2(taskPickupScans.taskId, task.id), eq2(taskPickupScans.boxId, box.id))).limit(1);
      if (duplicateScan.length > 0) {
        return res.status(409).json({ error: "Diese Box wurde f\xFCr diese Aufgabe bereits gescannt." });
      }
      const txResult = await db.transaction(async (tx) => {
        const removal = await removeActivePlacement({
          box,
        ...
        const [scanRecord] = await tx.insert(taskPickupScans).values({
          taskId: task.id,
          standId: stand.id,
          boxId: box.id,
          scannedByUserId: authUser.id
        }).returning();
        const [updatedItem] = await tx.update(taskPickupItems).set({
          scannedCount: Math.min(pickupItem.scannedCount + 1, pickupItem.requiredCount),
          updatedAt: /* @__PURE__ */ new Date()
        }).where(eq2(taskPickupItems.id, pickupItem.id)).returning();
        const remaining = await tx.select({ pending: count() }).from(taskPickupItems).where(
          and2(
            eq2(taskPickupItems.taskId, task.id),
            sql3`${taskPickupItems.scannedCount} < ${taskPickupItems.requiredCount}`
          )
        );
        let updatedTask = void 0;
        if (Number(remaining[0]?.pending || 0) === 0 && task.status === "OPEN") {
          [updatedTask] = await tx.update(tasks).set({
            status: "PICKED_UP",
            pickedUpAt: /* @__PURE__ */ new Date(),
            claimedByUserId: authUser.id,
            claimedAt: /* @__PURE__ */ new Date(),
            updatedAt: /* @__PURE__ */ new Date()
          }).where(eq2(tasks.id, task.id)).returning();
        }
        return { scanRecord, updatedItem, removal, updatedTask };
      });
      const standMeta = await buildStandContextMeta(stand.id);
      await createAuditEvent({
        taskId: task.id,
        ...
        entityType: "box",
        entityId: box.id,
        beforeData: { standId: placement.standId, status: box.status },
        afterData: { standId: null, status: "IN_TRANSIT" },
        metaJson: { ...standMeta, boxId: box.id, source: "TASK_PICKUP_SCAN" }
      });
      if (txResult.updatedTask) {
        await createAuditEvent({
          taskId: task.id,
          actorUserId: authUser.id,
          action: "STATUS_PICKED_UP",
          entityType: "task",
          entityId: task.id,
      ...
      const items = await db.select().from(taskPickupItems).where(eq2(taskPickupItems.taskId, task.id));
      const progress = items.reduce(
        (acc, item) => {
          acc.required += item.requiredCount;
          acc.scanned += item.scannedCount;
          return acc;
        },
        { required: 0, scanned: 0 }
      );
      res.json({
        message: "Pickup-Scan gespeichert",
        scan: txResult.scanRecord,
        pickupItem: txResult.updatedItem,
        taskStatus: txResult.updatedTask?.status || task.status,
        progress
      });
    } catch (error) {
      console.error("Failed to handle pickup scan:", error);
      const status = error?.error?.status || 500;
      const message = error?.error?.message || "Pickup-Scan fehlgeschlagen";
  ...
  app2.post("/api/scan/place-box", requireAuth, async (req, res) => {
    try {
      ...
      await createAuditEvent({
        taskId: box.currentTaskId || box.id,
        actorUserId: authUser.id,
        action: placementResult.alreadyPlaced ? "BOX_POSITIONED" : "BOX_PLACED",
        entityType: "box",
        entityId: box.id,
        beforeData: { standId: box.standId, status: box.status },
        afterData: { standId: stand.id, status: "AT_STAND" },
      ...
      if (box.currentTaskId) {
        ...
        if (task && (task.status === "PICKED_UP" || task.status === "IN_TRANSIT")) {
          try {
            ...
            if (timestampField) {
              taskUpdate[timestampField] = /* @__PURE__ */ new Date();
            }
            await db.update(tasks).set(taskUpdate).where(eq2(tasks.id, task.id));
            await createAuditEvent({
              taskId: task.id,
              actorUserId: authUser.id,
              action: "STATUS_CHANGED",
  ...
  app2.post("/api/scan/pickup-box", requireAuth, async (req, res) => {
    try {
      const { boxQr, standQr, geo, locationDetails } = req.body;
      const authUser = req.authUser;
      if (!boxQr) {
      ...
      const placement = await getActivePlacementForBox(box.id);
      if (!placement) {
        return res.status(409).json({ error: "Diese Box ist aktuell keinem Stellplatz zugeordnet." });
      }
      const previousStandId = placement.standId;
      ...
      if (standQr) {
        const [scannedStand] = await db.select().from(stands).where(eq2(stands.qrCode, standQr));
        if (!scannedStand) {
          return res.status(404).json({ error: "Stellplatz nicht gefunden" });
        }
        if (scannedStand.id !== placement.standId) {
          return res.status(409).json({ error: "Diese Box geh\xF6rt aktuell nicht zu diesem Stellplatz/St\xE4nder." });
        }
      }
      const beforeData = { standId: placement.standId, status: box.status };
      const removal = await removeActivePlacement({ box, standId: placement.standId, userId: authUser.id });
      if ("error" in removal) {
        return res.status(removal.error.status).json({ error: removal.error.message });
      }
      const updatedBox = removal.updatedBox;
      const [scanEvent] = await db.insert(scanEvents).values({
        containerId: box.id,
        containerType: "box",
        taskId: box.currentTaskId || null,
